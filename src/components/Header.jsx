    ...
    return (
        <header className={`transition-colors duration-300 h-28 fixed top-0 left-0 right-0 z-20 ${linkColor} ${getScrollDirection(scrollDir)}`}>
            <nav className="w-full h-full p-6 lg:px-8 flex items-center justify-between" aria-label="Global">
                ...
                <button
                    type="button"
                    className="xl:hidden -m-2.5 inline-flex items-center justify-center rounded-md p-2.5  hover:text-link transition-colors"
                    onClick={() => setMobileMenuOpen(true)}
                >
                    <span className="sr-only">Open main menu</span>
                    <Bars3Icon className="h-6 w-6" aria-hidden="true" />
                </button>
                <div className="px-8 hidden xl:flex sm:gap-x-4 md:gap-x-12 lg:gap-x-24">
                    {navigation.map((item) => (
                        <NavLink key={item.name} to={item.to} className={({ isActive }) => "text-lg lg:text-xl font-normal leading-6  hover:text-link transition-colors" + (isActive ? " text-link" : "")}>
                            {item.name}
                        </NavLink>
                    ))}
                </div>
