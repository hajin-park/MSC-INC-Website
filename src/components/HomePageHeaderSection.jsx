...
export default function HomePageHeaderSection() {
    ...
    return (
            <div className="relative w-full h-screen sm:h-236 md:h-192 lg:h-236 p-16 lg:p-40 flex flex-col items-center justify-center text-center bg-gradient-to-b from-black/80 to-black/10 text-white">
                ...
                <button onClick={handlePrev} className="transition-opacity duration-500 opacity-0 hover:opacity-100 absolute flex justify-start items-center w-1/3 h-full left-0 top-1/2 transform -translate-y-1/2 z-10 hover:bg-gradient-to-r from-black/50 to-black/0">
                    <ChevronLeftIcon className="h-12 w-12 text-white" />
                </button>
                <button onClick={handleNext} className="transition-opacity duration-500 opacity-0 hover:opacity-100 absolute flex justify-end items-center w-1/3 h-full right-0 top-1/2 transform -translate-y-1/2 z-10 hover:bg-gradient-to-l from-black/50 to-black/0 ">
